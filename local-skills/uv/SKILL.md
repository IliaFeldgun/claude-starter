...
| Isolated env for a project under `/workspace` | `/opt/uv-envs/<project>` | `uv venv --python 3.12 /opt/uv-envs/<project>` |
| Tool you don't want installed | nowhere — uv manages it | `uvx <tool>` |
| `uv sync` / `uv run` against a project in `/workspace` | `/opt/uv-envs/<project>` | `UV_PROJECT_ENVIRONMENT=/opt/uv-envs/<project> uv sync` |
...
| Wrong | Right |
| --- | --- |
| `cd /workspace/proj && uv venv .venv` | `uv venv /opt/uv-envs/proj` |
| `cd /workspace/proj && uvx <tool> <path>` | `uvx <tool> proj/<path>` from your existing cwd, or an absolute path — just don't move the shell |
| `cd /workspace/proj && uv sync` | `UV_PROJECT_ENVIRONMENT=/opt/uv-envs/proj uv sync --project /workspace/proj` (or a relative `--project` from where you are) |
...
| Lint/typecheck a `/workspace` project with reqs in `requirements.txt` | `uvx --with-requirements /workspace/path/to/requirements.txt pyright /workspace/path/to/pkg/` |
| Lint/typecheck a project with `pyproject.toml` | `UV_PROJECT_ENVIRONMENT=/opt/uv-envs/<name> uv run --project /workspace/path/to/proj pyright pkg/` |
| Install into the shared `/opt/venv` | `uv pip install httpx` |
| Sync a project's lockfile into a relocated venv | `UV_PROJECT_ENVIRONMENT=/opt/uv-envs/<name> uv sync --project /workspace/path/to/proj` |
| Install requirements.txt into a fresh isolated env | `uv venv --python 3.12 /opt/uv-envs/<name> && uv pip install --python /opt/uv-envs/<name>/bin/python -r /workspace/.../requirements.txt` |
...
- **Don't** `cd` into a `/workspace` project and run `uv sync` / `uv venv .venv` — that drops a `.venv/` into the Mac bind-mount. Use `UV_PROJECT_ENVIRONMENT=/opt/uv-envs/<name>` with `--project` (or `--directory`) instead.
- If you see a pre-existing `.venv/` in a `/workspace` project, it was likely created on the host (Mac); its Python interpreter symlinks point at `/opt/homebrew/...` and are dead inside this container. Don't try to reuse it — its `bin/python` won't execute.

...
- **Never put a venv under the mounted project dir or `/home/ubuntu`.** The project dir (mounted at a `/WORKSPACE`-prefixed path named after the launch directory, e.g. `/WORKSPACE/myproj`) is the user's Mac bind-mount; `/home/ubuntu` is a persistent VM volume. Both survive container rebuilds. Venvs are large, Python-version-specific, and often hold host-incompatible symlinks (a `.venv` created on the Mac will have dead Homebrew Python symlinks inside this Linux container). Put per-project venvs under `/opt/uv-envs/<name>` — that path is baked into the image as ubuntu-writable, lives on the ephemeral overlay, and gets wiped on container restart.
- **Never `cd` to invoke a uv/python command.** Your cwd is already the mounted project root — run `pwd` to confirm, then reference files by relative path from there (`requirements.txt`, `pkg/`, `.`). Relative paths are shorter, read cleaner, and stay runnable on the user's Mac (the project dir is a bind-mount). For a path outside cwd, pass it as a relative or absolute argument, or use the tool's own `--project` / `--directory` / `--cwd` flag — don't move the shell.
...
| Wrong | Right |
| --- | --- |
| `cd <project> && uv venv .venv` | `uv venv /opt/uv-envs/proj` |
| `cd <subdir> && uvx <tool> file.py` | `uvx <tool> <subdir>/file.py` (relative path from cwd, no shell move) |
| `cd <project> && uv sync` | cwd is already the project root — `UV_PROJECT_ENVIRONMENT=/opt/uv-envs/proj uv sync` |
...
| Lint/typecheck a mounted project with reqs in `requirements.txt` | `uvx --with-requirements requirements.txt pyright pkg/` (from project root) |
| Lint/typecheck a project with `pyproject.toml` | `UV_PROJECT_ENVIRONMENT=/opt/uv-envs/<name> uv run pyright pkg/` (from project root) |
| Install into the shared `/opt/venv` | `uv pip install httpx` |
| Sync a project's lockfile into a relocated venv | `UV_PROJECT_ENVIRONMENT=/opt/uv-envs/<name> uv sync` (from project root) |
| Install requirements.txt into a fresh isolated env | `uv venv --python 3.12 /opt/uv-envs/<name> && uv pip install --python /opt/uv-envs/<name>/bin/python -r requirements.txt` |
